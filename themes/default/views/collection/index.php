<script language="javascript">
	...
	<?php foreach ($nativeFields as $field): if ($field == "_id") {
		continue;
	} ?>
...
<div class="operation">
	<strong><?php hm("query"); ?></strong>[<a
	href="<?php h($arrayLink);?>" <?php if (x("format") == "array"): ?>style="text-decoration:underline"<?php endif;?>>Array</a>|<a
	href="<?php h($jsonLink); ?>" <?php if (x("format") != "array"): ?>style="text-decoration:underline"<?php endif;?>>JSON</a></a>
	] |  <?php if ($_logQuery): ?><a href="#" onclick="showQueryHistory();return false;">History</a> | <?php endif;?>
...
<div class="query">
	<form method="get" id="query_form">
		...
		<table>
			<tr>
				<td valign="top">
					<textarea name="criteria" rows="5" cols="70"
					          style="height:100px"><?php h(x("criteria"));?></textarea><br/>

					<div id="newobjInput" <?php if (x("command") != "modify"): ?>style="display:none"<?php endif;?>>
						New Object(see <a href="http://www.mongodb.org/display/DOCS/Updating"
						                  target="_blank">Updating</a> operators):<br/>
				...
				<td valign="top" class="field_orders">
					...
					<p><input type="text" name="field[]" value="<?php h(rock_array_get(x("field"), 0));?>"/> <select
						name="order[]">
						<option
							value="asc" <?php if (rock_array_get(x("order"), 0) == "asc"): ?>selected="selected"<?php endif;?>>
							ASC
						</option>
						<option
							value="desc" <?php if (rock_array_get(x("order"), 0) == "desc"): ?>selected="selected"<?php endif;?>>
							DESC
						</option>
					</select></p>
					<p><input type="text" name="field[]" value="<?php h(rock_array_get(x("field"), 1));?>"/> <select
						name="order[]">
						<option
							value="asc" <?php if (rock_array_get(x("order"), 1) == "asc"): ?>selected="selected"<?php endif;?>>
							ASC
						</option>
						<option
							value="desc" <?php if (rock_array_get(x("order"), 1) == "desc"): ?>selected="selected"<?php endif;?>>
							DESC
						</option>
					</select></p>
					<p><input type="text" name="field[]" value="<?php h(rock_array_get(x("field"), 2));?>"/> <select
						name="order[]">
						<option
							value="asc" <?php if (rock_array_get(x("order"), 2) == "asc"): ?>selected="selected"<?php endif;?>>
							ASC
						</option>
						<option
							value="desc" <?php if (rock_array_get(x("order"), 2) == "desc"): ?>selected="selected"<?php endif;?>>
							DESC
						</option>
					</select></p>
					<p><input type="text" name="field[]" value="<?php h(rock_array_get(x("field"), 3));?>"/> <select
						name="order[]">
						<option
							value="asc" <?php if (rock_array_get(x("order"), 3) == "asc"): ?>selected="selected"<?php endif;?>>
							ASC
						</option>
						<option
							value="desc" <?php if (rock_array_get(x("order"), 3) == "desc"): ?>selected="selected"<?php endif;?>>
							DESC
						</option>
					</select></p>
				</td>
			...
			<?php if (!empty($nativeFields)): ?>
				<a href="#" onclick="showQueryFields(this);return false;" title="Choose fields to display">Fields(<span
					id="query_fields_count"><?php h(count($queryFields));?></span>) <span
					style="font-size:11px">▼</span></a> |
				<a href="#" onclick="showQueryHints(this);return false;" title="Choose indexes will be used in query">Hints(<span
					id="query_hints_count"><?php h(count($queryHints));?></span>) <span style="font-size:11px">▼</span></a>
				|
				<?php endif; ?>
			</span>
					...
					<label
						id="limitLabel" <?php if (x("command") != "findAll"): ?>style="display:none"<?php endif;?>><?php hm("limit"); ?>
						:<input type="text" name="limit" size="5" value="<?php h(xi("limit"));?>"/> |</label>
			...
			<select name="pagesize" title="<?php hm("rows_per_page"); ?>">
				<?php foreach (array(0, 10, 15, 20, 30, 50, 100, 200, 500, 1000) as $pagesize): ?>
				<option
					value="<?php h($pagesize);?>" <?php if (x("pagesize") == $pagesize): ?>selected="selected"<?php endif;?>>
					Rows:<?php h($pagesize);?>
				</option>
				<?php endforeach;?>
			</select> |</span>
					...
					<select name="command" onchange="changeCommand(this)">
						<option
							value="findAll" <?php if (x("command") == "findAll"): ?>selected="selected"<?php endif;?>>
							findAll
						</option>
						<option value="remove" <?php if (x("command") == "remove"): ?>selected="selected"<?php endif;?>>
							remove
						</option>
						<option value="modify" <?php if (x("command") == "modify"): ?>selected="selected"<?php endif;?>>
							modify
						</option>
					</select>
				</td>
			...
			<tr>
				<td colspan="2">
					...
					<input type="button" value="<?php hm("clear_conditions"); ?>"
					       onclick="window.location='<?php h(url("collection.index", array("db" => $db, "collection" => $collection, "format" => xn("format")))); ?>'"/>
		...
		<div id="query_fields_list" class="fieldsmenu">
			<div align="right" style="padding-right:10px;background:#999">
				<a href="#" onclick="closeQueryFields();return false;" title="Click to close">
					<img src="<?php render_theme_path() ?>/images/accept.png" width="14"/>
				</a>
			</div>
			<ul>
				<?php foreach ($nativeFields as $field): if ($field == "_id") {
		...
		<div id="query_hints_list" class="fieldsmenu">
			<div align="right" style="padding-right:10px;background:#999"><a href="#"
			                                                                 onclick="closeQueryHints();return false;"
			                                                                 title="Click to close"><img
				src="<?php render_theme_path() ?>/images/accept.png" width="14"/></a></div>
			<ul>
				<?php foreach ($indexFields as $index => $field): ?>
				<li title="<?php h(htmlspecialchars($field["keystring"])); ?>"><label><input type="checkbox"
				                                                                             name="query_hints[<?php h($index); ?>]"
				                                                                             value="<?php h($field["name"]); ?>" <?php if (in_array($field["name"], $queryHints)): ?>checked="checked"<?php endif;?>
				                                                                             class="query_hints"/> <?php h($field["name"]); ?>
				</label></li>
				<?php endforeach; ?>
			</ul>
		</div>
...
<div id="records">
	...
	<?php if (x("command") != "findAll"): ?>
		...
		<?php endif; ?>
	<?php else: ?>
	<p class="page"><?php h($page); ?> (<?php h(min($page->total(), $page->offset() + $page->size()));?>
		/ <?php h($page->total());?>)</p>

	<!-- list all records -->
	<?php foreach ($rows as $index => $row): ?>
		<div style="border:2px #ccc solid;margin-bottom:5px;" onmouseover="showOperationButtons('<?php h($index);?>')"
		     onmouseout="hideOperationButtons('<?php h($index);?>')" class="record">
				...
				<tr>
					...
					<td valign="top">
						...
						<div class="operation" id="operate_<?php h($index);?>">
							...
							<a href="#"
							   onclick="fieldOpNew(null,'<?php h(rock_id_string($row["_id"])); ?>','',<?php h($index); ?>);return false;"><?php echo hm("new_field"); ?></a>
							|
							<?php else: ?>
							...
							<a href="#"
							   onclick="refreshRecord('<?php h(rock_id_string($row["_id"])) ?>',<?php h($index); ?>);return false;"><?php hm("refresh"); ?></a>
							|
							<?php endif; ?>
	...
	"files_id" => ' . (($row["_id"] instanceof MongoId) ? "new MongoId(\"" . addslashes($row["_id"]->__toString()) . "\")" : "\"" . addslashes($row["_id"]) . "\"") . '
						...
						<!-- display record -->
						<div id="text_<?php h($index);?>" style="max-height:150px;overflow-y:hidden;width:99%"
						     ondblclick="expandText('<?php h($index);?>');" class="record_row"
						     record_id="<?php if (isset($row["_id"])) {
							     h(rock_id_string($row["_id"]));
						     } ?>" record_index="<?php h($index); ?>">

							<?php h($row["data"]); ?>
						</div>

						<!-- switch to text so we can copy it easieer -->
						<div id="field_<?php h($index);?>" style="display:none;max-height:150px;overflow-y:auto">
							<textarea rows="7" cols="60" ondblclick="this.select()"
							          title="Double click to select all"><?php h($row["text"]);?></textarea></div>
...
<div id="field_dialog_new" style="display:none">
	<table>
		...
		<tr>
			<td>Data Type:</td>
			<td><?php render_select_data_types("data_type", "string"); ?> <select name="format"
			                                                                      onchange="switchDataFormat('#field_dialog_new textarea[name=\'mixed_value\']',this)"
			                                                                      style="display:none">
				<option value="array" <?php if ($last_format == "array"): ?>selected="selected"<?php endif; ?>>Array
				</option>
				<option value="json" <?php if ($last_format == "json"): ?>selected="selected"<?php endif; ?>>JSON
				</option>
			</select></td>
		</tr>
		<tr class="value">
		...
		<tr class="mixed_value">
			...
			<td><textarea name="mixed_value" rows="10" cols="50"><?php if ($last_format == "array"): ?>array(
				...
				}<?php endif; ?></textarea><br/> * An array or object
			</td>
		</tr>
	</table>
</div>
...
<div id="field_dialog_update" style="display:none">
	<table>
		...
		<tr>
			<td>Data Type:</td>
			<td><?php render_select_data_types("data_type", "integer"); ?> <select name="format"
			                                                                       onchange="switchDataFormat('#field_dialog_update textarea[name=\'mixed_value\']',this)"
			                                                                       style="display:none">
				<option value="array" <?php if ($last_format == "array"): ?>selected="selected"<?php endif; ?>>Array
				</option>
				<option value="json" <?php if ($last_format == "json"): ?>selected="selected"<?php endif; ?>>JSON
...
<div id="field_dialog_query" style="display:none">
	<table>
		<tr>
			<td valign="top">Criteria <select name="format"
			                                  onchange="switchDataFormat('#field_dialog_query textarea[name=\'field_criteria\']',this)">
				<option value="array" <?php if ($last_format == "array"): ?>selected="selected"<?php endif; ?>>Array
				</option>
				<option value="json" <?php if ($last_format == "json"): ?>selected="selected"<?php endif; ?>>JSON
				</option>
...
<div id="field_dialog_indexes" style="display:none">
	...
	<table bgcolor="#cccccc" width="400" cellpadding="2" cellspacing="1">
		...
		<tr bgcolor="#ffffff">
			...
			<td><input type="text" name="field[]" class="first_field"/> <select name="order[]">
				<option value="asc">ASC</option>
				<option value="desc">DESC</option>
			</select> <input type="button" value="+" onclick="addNewField()"/>
