...
<form method="post">
	...
	<div id="arguments" <?php if (empty($arguments)): ?>style="display:none"<?php endif;?>>
		...
		<?php foreach ($arguments as $index => $argument): ?>
			<div>
				<?php hm("argument"); ?> #<span><?php h($index);?></span>[JSON] <a href="#"
				                                                                   onclick="removeArgument(this);return false"><?php hm("remove"); ?></a><br/><textarea
				name="argument[]" rows="5" cols="60"><?php h($argument);?></textarea><br/>
			</div>
			<?php endforeach; ?>
		<?php endif;?>
	</div>
