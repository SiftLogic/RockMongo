...
<table bgcolor="#cccccc" cellpadding="2" cellspacing="1" width="600">
	...
	<tr bgcolor="#fffeee">
		...
		<td valign="top" <?php if (isset($prog["query"]) && $param == "opid"): ?>rowspan="2"<?php endif; ?>>
			...
			<?php if ($param == "opid"): ?>
				<?php h($prog["opid"]); ?><?php if ($prog["opid"] > 1): ?> [<a
					href="<?php h(url("server.killOp", array("opid" => $prog["opid"]))); ?>"
					onclick="return window.confirm('<?php hm("killop"); ?> \'<?php h($prog["opid"]);?>\'?')"><?php hm("kill"); ?></a>
					]<?php endif; ?>
				<?php
			else:
				if ($prog[$param] == "(NONE)" || $prog[$param] == "none") {
				} else {
					h($prog[$param]);
				}
			endif;
