...
<a href="http://www.mongodb.org/display/DOCS/List+of+Database+Commands"
   target="_blank">&raquo; <?php hm("listdbcommands"); ?></a>
...
<form method="post">
	...
	<select name="db">
		...
		<option
			value="<?php h($value["name"]);?>" <?php if (xn("db") == $value["name"]): ?>selected="selected"<?php endif;?>><?php h($value["name"]);?></option>
		<?php endforeach;?>
	</select>
	<?php hm("format"); ?>:<select name="format">
	<?php foreach (array("json" => "JSON", "array" => "Array") as $param => $value): ?>
	<option
		value="<?php h($param);?>" <?php if (x("format") == $param): ?>selected="selected"<?php endif;?>><?php h($value);?></option>
	<?php endforeach;?>
</select>
	<br/>
